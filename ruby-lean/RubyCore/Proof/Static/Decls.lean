...
— `heapOkB` decides it and `scripts/probes/heapok_probe.lean` reports it, so F0 is
...
def TyClass (h : Heap) (τ : Ty) (k : ObjId) : Prop :=
  ...
  | .union _ _ => False
  -- L270: an arrow dispatches from no method table; `tyClassNames` is `[]` at both
  -- spine arms and `DeclsOk` obliges nothing.
  | .arrow0 _ => False
  | .arrowCons _ _ => False
    ...
    have hτ : τr = .cls c ∧ decl = { params := [], ret := τb } := by
      ...
      | union a b =>
        ...
        rw [show declFor (addRow D c mname { params := [], ret := τb }) (.union a b) mname
            = none from by simp [declFor, tyClassNames]] at hdecl
        exact absurd hdecl (by simp)
      -- L270: and at the two arrow-spine arms.
      | arrow0 r =>
        exfalso
        rw [show declFor (addRow D c mname { params := [], ret := τb }) (.arrow0 r) mname
            = none from by simp [declFor, tyClassNames]] at hdecl
        exact absurd hdecl (by simp)
      | arrowCons pa r =>
        exfalso
        rw [show declFor (addRow D c mname { params := [], ret := τb }) (.arrowCons pa r) mname
            = none from by simp [declFor, tyClassNames]] at hdecl
        exact absurd hdecl (by simp)
      | sym =>
        exfalso
...
theorem declFor_addRow_self_inv {D : Decls} {c name : String} {σ : MethodDecl} {τr : Ty}
  ...
  | arrow0 _ => rw [hnil _ (by simp [tyClassNames])] at hdecl; exact absurd hdecl (by simp)
  | arrowCons _ _ => rw [hnil _ (by simp [tyClassNames])] at hdecl; exact absurd hdecl (by simp)
  | cls n =>
    by_cases hng : groundClassNames.contains n = true
...
certificate (`heapOkB`, `scripts/probes/heapok_probe.lean`) are stated over it. What
...
/-- Every entry of `baseDecls` still resolves in this heap, spelled out at the
    ...
    **This is no longer what the invariant carries** (F1a): `DeclsOk` is, and this
    ...
    reflects it into a `Bool`, `scripts/probes/heapok_probe.lean` reports it per clause,
...
def NoHook (h : Heap) : Prop :=
  (h.classPayload? Boot.objectId).isSome ∧
    ...
    -- **The module half of J44.** A freshly-allocated *module*'s eigenclass
    -- bottoms at `Module`, not `Class` (`RubyCore.Interp.eigenclassOf`'s
    -- `isModule` arm) — a different tail, so it needs its own clause rather than
    -- riding on the one above (`Module` is *in* `Class`'s chain, but the
    -- implication doesn't run backwards: nothing here says `Class`'s chain is
    -- hook-free *because* `Module`'s is).
    ∀ j ∈ ancestors h Boot.moduleId, ∀ cp, h.classPayload? j = some cp →
      ∀ n ∈ hookFreeNames, cp.methods.find? (·.1 == n) = none
...
theorem NoHook_defineMethod {h : Heap} {cls : ObjId} {name : String} {md : MethodDef}
  ...
  have hnn : ∀ n ∈ hookFreeNames, ¬ (n = name) := by
    intro n hn
  ...
  refine ⟨by rw [classPayload?_isSome_defineMethod]; exact hh.1, fun k hk n hn => ?_,
    fun j hjm cp hj n hn => ?_, fun j hjm cp hj n hn => ?_⟩
  · have hk' : (h.classPayload? k).isSome := by
    ...
    exact hh.2.1 k hk' n hn
  · have hmm := methods_find_defineMethod h cls j name n md (hnn n hn)
    rw [hj] at hmm
    cases hj0 : h.classPayload? j with
    | none => rw [hj0] at hmm; exact absurd hmm (by simp)
    | some cp0 =>
      rw [hj0] at hmm
      simp only [Option.map_some, Option.some.injEq] at hmm
      rw [hmm]
      exact hh.2.2.1 j (by rwa [ancestors_defineMethod] at hjm) cp0 hj0 n hn
  · have hmm := methods_find_defineMethod h cls j name n md (hnn n hn)
    rw [hj] at hmm
    cases hj0 : h.classPayload? j with
    | none => rw [hj0] at hmm; exact absurd hmm (by simp)
...
theorem noHookB_sound {h : Heap} (hb : noHookB h = true) : NoHook h := by
  ...
  obtain ⟨⟨⟨hobj, hb⟩, htbC⟩, htbM⟩ := hb
  refine ⟨hobj, fun k hk n hn => ?_, fun j hjm cp hj n hn => ?_, fun j hjm cp hj n hn => ?_⟩
  ...
  · have := List.all_eq_true.mp htbC j hjm
    rw [hj] at this
    ...
    · exact this.1
    · exact this.2
  · have := List.all_eq_true.mp htbM j hjm
    rw [hj] at this
    simp only [Bool.and_eq_true, Option.isNone_iff_eq_none] at this
    simp only [hookFreeNames, List.mem_cons, List.not_mem_nil, or_false] at hn
    rcases hn with rfl | rfl
...
def LitClsOk (h : Heap) : Prop :=
  ((h.classPayload? Boot.stringId).isSome ∧ className h Boot.stringId = "String") ∧
    ...
    -- J53: `Object`'s eigenclass is realized (boot does it eagerly since J53's
    -- groundwork), which is what makes the fresh-class composite two
    -- allocations: `eigenclassOf.go`'s superclass walk stops at `Object`.
    -- Presence of the payload rides along for the bound.
    ((h.classPayload? Boot.objectId).isSome ∧ (h.get Boot.objectId).eigen.isSome)
...
theorem LitClsOk_defineMethod {h : Heap} {cls : ObjId} {name : String}
  ...
  ⟨⟨by rw [classPayload?_isSome_defineMethod h cls Boot.stringId name md]; exact hs.1.1,
   ...
   ⟨by rw [classPayload?_isSome_defineMethod h cls Boot.regexpId name md]; exact hs.2.2.2.2.1.1,
    by rw [className_defineMethod h cls Boot.regexpId name md]; exact hs.2.2.2.2.1.2⟩,
   ⟨by rw [classPayload?_isSome_defineMethod h cls Boot.objectId name md]; exact hs.2.2.2.2.2.1,
    by rw [get_defineMethod_eigen h cls name md Boot.objectId]; exact hs.2.2.2.2.2.2⟩⟩
...
theorem tableOk_declsOk {h : Heap} (ht : TableOk h) (hcls : ClassOk h) :
  ...
  | arrow0 _ => exact absurd hd (by simp [declFor, tyClassNames])
  | arrowCons _ _ => exact absurd hd (by simp [declFor, tyClassNames])
...
theorem noHook (hi : IvarOnly h h') (hn : NoHook h) : NoHook h' :=
  ⟨by rw [hi.classPayload]; exact hn.1,
   ...
   fun j hjm cp hj n hnn => hn.2.2.1 j (by rwa [hi.ancestors_eq] at hjm) cp
     (by rw [← hi.classPayload]; exact hj) n hnn,
   fun j hjm cp hj n hnn => hn.2.2.2 j (by rwa [hi.ancestors_eq] at hjm) cp
     (by rw [← hi.classPayload]; exact hj) n hnn⟩
...
theorem litClsOk (hi : IvarOnly h h') (hs : LitClsOk h) : LitClsOk h' :=
  ⟨⟨by rw [hi.classPayload]; exact hs.1.1, by rw [hi.className_eq]; exact hs.1.2⟩,
   ...
   ⟨by rw [hi.classPayload]; exact hs.2.2.2.2.1.1,
    by rw [hi.className_eq]; exact hs.2.2.2.2.1.2⟩,
   ⟨by rw [hi.classPayload]; exact hs.2.2.2.2.2.1,
    by rw [hi.eigen]; exact hs.2.2.2.2.2.2⟩⟩
...
theorem noHook_constSetIn (hn : NoHook h) : NoHook (constSetIn h j nm v) := by
  refine ⟨by rw [classPayload?_isSome_constSetIn]; exact hn.1, fun k hk n hnn => ?_,
    fun i him cp hi n hnn => ?_, fun i him cp hi n hnn => ?_⟩
  ...
  · have hmm := methods_constSetIn h j i nm v
    rw [hi] at hmm
    cases hi0 : h.classPayload? i with
    | none => rw [hi0] at hmm; exact absurd hmm (by simp)
    | some cp0 =>
      rw [hi0] at hmm
      simp only [Option.map_some, Option.some.injEq] at hmm
      rw [hmm]
      exact hn.2.2.1 i (by rwa [ancestors_constSetIn] at him) cp0 hi0 n hnn
  · have hmm := methods_constSetIn h j i nm v
    rw [hi] at hmm
    cases hi0 : h.classPayload? i with
    | none => rw [hi0] at hmm; exact absurd hmm (by simp)
    | some cp0 =>
      ...
      simp only [Option.map_some, Option.some.injEq] at hmm
      rw [hmm]
      exact hn.2.2.2 i (by rwa [ancestors_constSetIn] at him) cp0 hi0 n hnn
...
theorem litClsOk_constSetIn (hs : LitClsOk h) : LitClsOk (constSetIn h j nm v) :=
  ⟨⟨by rw [classPayload?_isSome_constSetIn]; exact hs.1.1,
   ...
   ⟨by rw [classPayload?_isSome_constSetIn]; exact hs.2.2.2.2.1.1,
    by rw [className_constSetIn]; exact hs.2.2.2.2.1.2⟩,
   ⟨by rw [classPayload?_isSome_constSetIn]; exact hs.2.2.2.2.2.1,
    by rw [(get_constSetIn_fields h j nm v Boot.objectId).2.2.1]
       exact hs.2.2.2.2.2.2⟩⟩

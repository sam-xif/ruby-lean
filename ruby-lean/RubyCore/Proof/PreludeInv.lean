...
1. **The certificate route** (§2, `inv_of_cert`). Reflect `HeapOk` into a `Bool`
   ...
   `scripts/probes/heapok_probe.lean` computes and `scripts/check-proofs.sh` fails on.
...
(`scripts/probes/heapok_probe.lean`: all three `Integer` builtins resolve as tabulated —
...
`Proof/`, so that `scripts/probes/heapok_probe.lean` computes *the* predicate these
...
theorem heapOkB_sound {h : Heap} (hb : heapOkB h = true) : HeapOk h := by
  ...
  obtain ⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨h1, h2⟩, h3⟩, hz⟩, hnh⟩, h5⟩, h6⟩, h7⟩, h6a⟩, h7a⟩, hpc⟩, hhc⟩, hrc⟩, hrn⟩, hoe1⟩, hoe2⟩, h8⟩ := hb
  exact ⟨⟨intResolvesB_sound h1, intResolvesB_sound h2, intResolvesB_sound h3,
    ...
    ⟨⟨h6, by simpa using h7⟩, ⟨h6a, by simpa using h7a⟩, hpc, hhc,
     ⟨hrc, by simpa using hrn⟩, ⟨hoe1, hoe2⟩⟩,
    classOkB_sound h8⟩

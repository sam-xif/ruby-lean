#!/usr/bin/env bash
# The **semantic ratchet**: how many of the judgment's rules have been discharged as
# a proof obligation over the semantic denotation of types, proved from the real `stepFn`.
#
# Two legs, in order:
#
#   1. The denotation's own gate. `Denote/Ty/Examples.lean` is a file of `#guard`s, each of which
...
#   2. The clink registry. `semladder` reports which of `Ratchet/Judgment/DJudge.lean`'s rules are in
...
echo "--- denotation gate (Denote/Ty/Examples.lean's #guards, elaborated) ---"
lake build Denote
echo

...
import Denote.Rules.Init.InitExpr
...
theorem body_sem : SemSafeCtxA entryCtx [] .ivar0 body .sym bodyCtx [] .ivar0 :=
  SemSafeCtxA.initDef (ps := params) (Ib := .ivar0) (τ := .bool)
    rfl rfl (by simp [params, FirstOrder, isAliasTy]) rfl rfl definitionInit.sem
...
theorem class_run {m : Machine} (hm : StateOk ctx0 [] .ivar0 m) :
  ...
  (SemSafeCtxA.classDecl body_sem (by decide)) m hm
...
theorem new_sem : SemSafeCtxA callerCtx [] .ivar0 newExpr
  ...
  exact hr.newInst (d := init) (ps := params) (.cons .truLit .nil rfl) rfl hc
    (by change init ∈ [init]; simp) rfl (by decide)
    (by change "FlagBox" ∈ ["FlagBox"]; simp) rfl
    (by simp [params, FirstOrder, isAliasTy]) rfl rfl finalInit.sem (by decide)

...
/-!
# `Denote/Join.lean` — the join, and what it means

`joinT` (`Ratchet/Ty.lean`) is the checker's **total** join: two branch types in, one type out,
...
`Ratchet/Ty.lean` derives `BEq Ty` **without** `LawfulBEq`, and `joinT`/`joinTy` are written
...
`deriving` clause. If `Ratchet/Ty.lean` ever adds `deriving LawfulBEq`, this section deletes.
-/

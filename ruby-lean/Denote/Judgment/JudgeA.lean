...
/-!
# `Denote/Judgment/JudgeA.lean` — the answer-typed semantic judgment for `DJudge`, and its obligations

`../../AGENTS.md` §The answer-typed design §3.1, at `Ratchet/Check/Check.lean`'s index shape. This
...
`Ratchet/Validate.lean` is imported transitively (via `Denote/Sem/Core/Boot.lean`) for exactly two
things: the **empty context value** `ctx0`, because `StateOk` is `Ctx`-indexed and `Ctx` lives
in `Ratchet/Static/`, and `stateOk_boot`, the `#guard`ed conformance witness for the
...
/-- **The answer-typed semantic judgment**, at `Ratchet/Check/Check.lean`'s shape.

`κ` is `ctx0` and the ivar spine is `.ivar0` because `DJudge` has neither and the fragment it
covers declares nothing (`Ratchet/Check/Check.lean` §"Not in the judgment"). `Γ` and `Γ'` are
...
The obligation below is the **invariant** instead (`Denote/Sem/Core/Invariant.lean` is the
...
**Before adding a rule, read `Ratchet/Check/Check.lean` §Authoring a rule.** Twice the obligation
...
* the **answer-typed half** needs `runA_pushK` (`Denote/Sem/Core/Answer.lean`), the answer-level
...
`runA_pushK` is proved in `Denote/Sem/Core/Answer.lean`. Sequence and conditional proofs live

import Denote.Ty.Arrow
import Denote.Ty.DenB

/-!
# `Denote/Ty/ArrowCheck.lean` — refuting an arrow by running it

`ArrowFlat` (`Denote/Ty/Arrow.lean`) quantifies over every argument in the domain and every
fuel, so no `Bool` decides it — `Denote/Ty/DenB.lean` answers `false` on both arrow arms for
...
* the call did not return a value within `fuel` (raised, diverged, gated, jumped) — the arrow
  says nothing about such a run (`Denote/Ty/Den.lean` §The arrow arm, point 1), **skip**;

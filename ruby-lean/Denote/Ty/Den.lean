...
**What a type *means*, as a predicate over the real machine.** `Ratchet/Static/` says
...
`Denote/Ty/DenB.lean`), and for every **first-order** type that is exactly the shape:
...
at length in `Denote/Ty/Apply.lean`: `Closure.captured` is a `FrameId`, so a closure's captured
...
> pushing or popping a frame (`Later`, `Denote/Ty/Ext.lean`), for every `a : A` and `b : B` at
> `m₂`, if calling
...
`Reaches` (`Denote/Ty/Apply.lean`) is that quantifier. `Later` is deliberately the weaker of the
...
falsity, which is the failure mode `Denote/Sem/Core/Boot.lean` exists to police. Demonstrated:

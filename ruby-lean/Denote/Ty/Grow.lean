...
/-!
# `Denote/Ty/Grow.lean` — a type's meaning survives an allocation

One theorem, `denM_ext`: if `m₂` is `m` after an allocation (`Ext`, `Denote/Ty/Ext.lean`), then
...
* **The arrow** — free, by `Ext.later` then `Later.trans`, because `denM`'s arrow arm was
  rewritten to quantify over `Later`-futures for exactly this purpose (`Denote/Ty/Den.lean` §The
  arrow arm point 5).

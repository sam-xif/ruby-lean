...
/-!
# `Denote/Ty/Local.lean` — rebinding a local, and what a type's meaning survives of it

`Denote/Ty/Ext.lean`/`Denote/Ty/Grow.lean` did allocation. This file does the other machine change
...
them like the rest — which is what `ConstScopeOk` (`Denote/Sem/Core/State.lean`) is stated over. -/
...
`Judge.vasgn` uses to decide which bindings to widen (`Ratchet/Lang/Ty.lean` §Stale closure

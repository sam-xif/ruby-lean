...
/-!
# `Denote/Ty/Arrow.lean` — the arrow, uncurried and stabilised

`Denote/Ty/Den.lean` states the arrow one parameter at a time, because `Ty`'s arrow is a
...
(`Denote/Ty/Apply.lean`) is the quantifier. Two things worth saying about it:
...
`Denote/Ty/Den.lean` §The arrow arm point 5 for why the quantifier is there (the arrow is the

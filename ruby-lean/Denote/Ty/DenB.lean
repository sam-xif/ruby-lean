...
/-!
# `Denote/Ty/DenB.lean` — the computable core

`Ty → Heap → Value → Bool`, the literal shape the design note asked for, and the shape that
can be `#eval`'d against a real booted heap (`Denote/Ty/Examples.lean`) or dropped into a
`decide`.
...
therefore permanent, and it is the same split `FirstOrder` (`Denote/Ty/Den.lean`) names.
...
scope lives in `Machine.frames` (`Denote/Ty/Apply.lean`). Given a machine there is nothing

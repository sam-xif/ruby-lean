import Denote.Ty.ArrowCheck
import Semantics.Interp

/-!
# `Denote/Ty/Examples.lean` — the denotation, against the real booted heap
...
This is the check that pins `Denote/Ty/Apply.lean`'s central claim — that a closure's captured
...
`arrowCheck` can only *refute* (`Denote/Ty/ArrowCheck.lean`), and both halves of that show up

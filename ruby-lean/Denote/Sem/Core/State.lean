...
import Denote.Sem.Names.GlobalConsts
...
incoming local environment and ivar spine, an outgoing pair of the same. `Denote/Ty/Den.lean`
...
`Denote/Ty/Apply.lean`'s `applyIn` move without the pushed frame: the frame stack is left exactly
...
(`.unsupported`) or jumps imposes no obligation, for the same reason `Denote/Ty/Den.lean`'s arrow
...
**`EnvOk` gives `Ty.sameAs` its meaning.** The alias type (`Ratchet/Lang/Ty.lean`, tier 12) is
...
`Denote/Ty/Apply.lean`'s `applyIn`: a machine that performs an implicit-self send with
...
def EnvOk (Γ : Env) (m : Machine) : Prop :=
  ...
  -- convenience. `Denote/Sem/Core/Boot.lean`'s boot machine satisfies it.
...
(`Denote/Ty/Den.lean` §The arrow arm point 5), and this is the *other* component that quantifies
...
`Denote/Ty/Val.lean`'s nominal probes (`classNamed?`, `isAName`, `isClassRefNamed`) all resolve a
...
It is satisfiable at the real booted machine and `Denote/Sem/Core/Boot.lean` proves it there, from a
...
`Denote/Sem/Core/Boot.lean`'s guard along with the other two. -/
...
decidable `Bool` at the booted heap (`Denote/Sem/Core/Boot.lean`) instead of a quantifier over
`String`. -/
...
`Denote/Ty/Ext.lean` on why a *dangling* reference is what makes that clause necessary).
...
does and does not buy, are in [`Denote/Sem/Core/Frame.lean`](Frame.lean); the definitions live here
...
a row added to that table is covered here the same day — and `Denote/Sem/Core/Boot.lean`'s `Bool` is
...
`Denote/Sem/Core/Boot.lean`'s `queryOkB`):
...
The theorem every allocating rung needs, and the one that pays for `Denote/Ty/Ext.lean` and
`Denote/Ty/Grow.lean`. Component by component, and the shape of each proof is the finding:
...
and `Ratchet/Lang/Ty.lean`'s `capStale` is what decides which recorded facts that falsifies. The

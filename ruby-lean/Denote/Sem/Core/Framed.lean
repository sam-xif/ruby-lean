...
Eight definitions, one per member of `Ratchet/Static/`'s mutual family, each with **the
...
`Ratchet/Static/` and its obligation changes with it; add a rule and the ratchet's
...
1. **Partial correctness.** Only runs that reach `.value` impose anything, for the reason
   `Denote/Sem/Core/State.lean`'s `Evals` docstring gives at length. Stuck-freedom is the other
...
2. **`τ` at the post-machine, `Γ`/`I` conformance at both ends.** Evaluating `e` can allocate,
   ...
   no longer exists. Same argument as the arrow's codomain (`Denote/Ty/Den.lean`).
...
`Denote/Judgment/JudgeA.lean`'s `SemJudgeA`, whose hypothesis is an **answer** and whose
...
`found-issues.md` §F27, `AGENTS.md`, and `Denote/Judgment/JudgeA.lean`'s header.

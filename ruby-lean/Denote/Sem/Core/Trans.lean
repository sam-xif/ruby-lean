...
/-!
# `Denote/Sem/Core/Trans.lean` — the syntax bridge

`Ratchet/Lang/Expr.lean` is `RubyCore/Syntax.lean`'s `Expr` **copied as text** (`AGENTS.md`
...
load-bearing: `Ratchet/` importing nothing from `RubyCore/` is what makes a divergence
...
`Denote/Ty/Den.lean` §Two stated gaps said `Ty.clos`'s `idx` has no denotation, because it
...
`toRuby` there is a coercion, so `Denote/Sem/Core/State.lean`'s `closTblOk` can compare a live

...
/-!
# `Denote/Sem/Core/Invariant.lean` — what an inductive invariant proving type safety looks like on
...
than forty arguments (this package's working rule — `Denote/Sem/Core/Frame.lean`'s `KontFrame`,
...
1. **`StateOk`** — `Denote/Sem/Core/State.lean`, 1633 lines, and every one of the 48 discharged
...
2. **`CtlOk`** — three arms, one per `Ctl`, and this is where the checker plugs in: the
   ...
   `Answer` (`Denote/Sem/Core/Answer.lean`), which is why this component is a definition rather
...
The obvious move is to reuse `Denote/Sem/Core/SafeKont.lean`: `Inv m := ∃ A, Delivers … A ∧ SafeKont
...
containing `catch` has a `catchK` on its continuation. `Denote/Sem/Core/AnswerCatch.lean` explains
...
* **`catchK` is never reached.** The tag index §3 argues `KontOk` must carry is required for
  ...
  vacuity risk `Denote/Sem/Core/Boot.lean` exists to answer on the other axis. Worth a corpus rung
...
what lets the reduction be instantiated at `Ratchet/Check/Check.lean`'s `DTyped` without this file
...
3. **`kont = []` vs `kont = K`, and this is the one that bites.** `SemJudge`'s hypothesis is
   ...
   `evalFrom` sets **`kont := []`** (`Denote/Sem/Core/State.lean`). So at a reachable machine the
   ...
   (`Denote/Sem/Core/AnswerCatch.lean` §`CatchFree` is the degenerate case of a protocol) is not an
...
proved for an *abstract* `Inv`, so it is exactly as usable against `Ratchet/Check/Check.lean`'s

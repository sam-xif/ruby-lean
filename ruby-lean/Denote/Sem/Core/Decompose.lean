...
/-!
# `Denote/Sem/Core/Decompose.lean` — the fifth stall point, cleared

`Denote/Sem/Core/Frame.lean` states the interpreter's frame rule (`KontFrameCatchFree`) and the
...
> induction** from `Denote/Sem/Core/Answer.lean`'s `run_pushK` — which is this induction with the
...
`Denote/Sem/Core/Frame.lean` defines `pushK` and `CatchFree` independently of

...
/-!
# `Denote/Sem/Core/Boot.lean` — the ladder is not vacuous
...
and the same one `Denote/Ty/Examples.lean`'s 31 guards observe. A boot failure falls back to a
...
(`Denote/Ty/Ext.lean`'s `get_oob` is the same fact the allocating rungs spend). -/
...
`Denote/Ty/Examples.lean`'s 31 guards have, and the same trade

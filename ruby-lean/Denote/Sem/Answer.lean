...
`run_split` (`Denote/Sem/Decompose.lean`) splits a run at the `.value` it delivers and needs
...
`Denote/Sem/Decompose.lean`'s `deliver`, which is the `val` arm. -/
...
/-- `stepFn`'s frame rule in `frameR` form — the shape `Denote/Sem/Decompose.lean` re-derived

...
reproducer in [`../../notes/ratchet/found-issues.md`](../../notes/ratchet/found-issues.md) §F1; the short form is that
...
### The `Builtins` **and `Interp/Support`** layers, proved *(clinks 52–53, `RubyCore/Proof/KontFrame.lean`)*
...
3. ~~**One `partial def` sits on the chain and blocks it outright**~~ — **fixed, clink 53.**
   ...
   `RubyCore/Proof/KontFrame.lean` records exactly what it needs. The original
...
Recorded here rather than in `../../notes/ratchet/found-issues.md` for the reproducer above, which is for
...
as `../../notes/ratchet/found-issues.md` §F3 — **fixed in clink 49**, conservatively: `declFree` filters
...
* `bareName` needed `lookup` to answer **nothing**, not "nothing of the user's" —
  ...
  one makes the miss *return*. That is `../../notes/ratchet/found-issues.md` §F4 — a wrong answer both
...
by a redefinition of `foo` is the shape — the same shape as `../../notes/ratchet/found-issues.md` §F1, with
...
invariant**. Recorded in `../../notes/ratchet/found-issues.md` §A6.
...
> and stayed red — three pre-existing breaks, listed in `../../notes/ratchet/found-issues.md` §A7. Every

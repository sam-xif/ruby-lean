...
- **L59 — fragment-coverage audit (2026-07-30) and the doc-staleness lesson.** The
  ...
  the reflection predicates and `attr_*` as pending (all three shipped); `ruby-lean/README.md`
  §Fragment listed `Array#each`/`map`, `Integer#times`, `Hash.new{}`, `attr_reader`,
  `include`, kwargs and Float formatting as *gated* (all work); `ruby-lean/notes/model/HANDOFF.md` still
  ...
  **Audit method, repeatable — prefer it over reading prose:**
  1. *Feature probe:* write a one-line snippet per feature, run
     `harness/desugar-dt/bin/export-json f.rb | ruby-lean/.lake/build/bin/rubycore`, and read
...
still to come. New directory `ruby-lean/RubyCore/Regex/`, nothing over 400 lines.
...
`scripts/probes/heapok_probe.lean` boots the prelude and asks the four clauses directly. All four hold:
...
3. **`TableOk` is kept, unchanged, and is now a *witness* rather than the invariant.**
   ...
   statement, `heapOkB`/`scripts/probes/heapok_probe.lean` decide the concrete one. **F0 needed no
...
(`scripts/probes/alloc_probe.lean`, kept as a check):
...
`heapOkB` still true at the booted heap; `scripts/probes/alloc_probe.lean` exit 0; tier-0 flat.
...
`scripts/probes/alloc_probe.lean` now counts what the clause costs at the *prelude-booted* heap:
...
heap; `scripts/probes/alloc_probe.lean` exit 0 with the new count; tier-0 flat at **992 agree, 0 disagree**.
...
at the boot heap. Forty lines of `IO` (`scripts/probes/ancestors_probe.lean`) before any proof attempt:
...
assertion: `scripts/probes/alloc_probe.lean` reports **0 of 105** booted objects refused, 18 plain
receivers.
...
the machine in hand** — it just decides one more clause. `scripts/probes/heapok_probe.lean` reports it;
`scripts/probes/ancestors_probe.lean` keeps the per-class diagnostic and the record that the *descent* route
is false.
...
* **Resolution** — `ResolvesAt` requires `fromPrelude = false`, and `prelude/prelude.rb` defines
  ...
  before adding a row; `scripts/probes/heapok_probe.lean` now reports the fourth name so the second
...
2. A `ClassGrow` relation. `PlainGrow`'s third clause — *nothing anywhere became a class* — is
   ...
   *no in-bounds object has an edge pointing out of bounds* clause `scripts/probes/ancestors_probe.lean`
...
The second was measured before it was assumed (`scripts/probes/names_probe.lean`, now part of
...
(`scripts/probes/names_probe.lean`):
...
`scripts/probes/consts_probe.lean` is in `check-proofs.sh` as its fifth measurement section.
...
`scripts/probes/reopen_probe.lean` decides `ClassOk`'s seven per-name conjuncts for every plausible candidate.
...
`scripts/probes/reopen_probe.lean` decides all seven `ClassOk` clauses per candidate name, so widening the
...
`scripts/probes/reopen_probe.lean` now decides both sets per candidate — the fifth probe extension whose
...
`scripts/probes/consts_probe.lean` runs it at the booted heap and `check-proofs.sh` fails on it. Measured:

...
local to `ruby-lean/` and the underlying problem is not.

Reproduce any `MODEL`/`DISAGREE` line with:

```sh
ruby X.rb                                                    # the oracle
desugar/bin/export-json X.rb | ruby-lean/.lake/build/bin/rubycore   # the model
```
...
copy of that probe drops the row for this reason (`ruby-lean/slice/drivers/slice-adversarial.rb`).
...
`not_BuiltinsSeal` is retired. See `ruby-lean/Denote/Sem/notes.md`, the eighteenth stall point,
...
`ruby-lean/Denote/Sem/Locals.lean`'s `Sealed` is. `Sealed.clos` reads `cl.captured`, so this
fiction breaks the seal at `b = 0` — the toplevel frame, the one every toplevel narrowing rung
is about. `ruby-lean/Denote/Sem/StepLocal.lean`'s `not_BuiltinsSeal` is the refutation, and
...
`RubyCore/Interp/`, 14 files under `RubyCore/Proof/`, 12 under `ruby-lean/Denote/`.
...
**Status:** fixed for `ruby-lean/`'s rungs only; **open for the shared harness.**
...
**Fixed in `ruby-lean/scripts/build_slice_rungs.py`'s `BLANK_NIL_STUB`** (the `nil_class` and
...
## §F28 — the strip stack mixed **byte** and **character** offsets, and corrupted every multi-byte file *(fixed)*

Found by the typed pipeline (`scripts/build_corpus.py`), which is the first thing to run
the whole strip stack over the `slice/` rungs. Four of them came out as **unparseable
Ruby**, and the first symptom was memorable: `require "sorbet-runtime"\nclass Module`
became `relass Module`.

**The bug.** Prism's `Location` offers both `start_offset` (a **byte** offset) and
`start_character_offset`. Ruby's `String#[]` indexes by **characters**. Three of the six
strip transforms took the byte offsets and spliced with `String#[]`:

| transform | offsets used | status |
|---|---|---|
| `sig_strip.rb` | `start_character_offset` | correct, its header even says so |
| `visibility_strip.rb` | `start_character_offset` | correct |
| `freeze_strip.rb` | `end_character_offset` | correct |
| `require_strip.rb` | `start_offset` | **wrong** |
| `const_inline.rb` | `start_offset` (splice *and* a `src[...]` read) | **wrong** |
| `class_sugar_strip.rb` | `start_offset` | **wrong** |

So the span deleted was shifted left by one character per multi-byte character earlier in
the file. An em dash in a generated header comment is enough, which is exactly what the
`slice/` rungs carry (`# Generated by ruby-lean/scripts/build_slice_rungs.py — do not
edit`). Every corpus rung is pure ASCII, which is why 259 of them never showed it.

**The fix**, in the three wrong files: do the splices on a `force_encoding(BINARY)` copy
and restore the encoding afterwards, so byte offsets and `String#[]` agree; and in
`const_inline.rb`, read the captured literal with `byteslice` rather than `[]`. Four
`slice/` rungs went from an unparseable stripped program to a clean one (upstream
failures 11 → 7), and agreement over the stripped corpus went from **248 agree + 4
control_invalid** to **252 agree, 0 disagreements**.

**What it did not affect, checked:** `sorbet-cert/run-all.sh` (6 programs, same verdicts)
and `certify/tests/jcert_test.sh` (6 cases, all ok). Both exercise the same stack on
ASCII inputs, where the two offset kinds coincide.

**The lesson worth keeping.** This is `certificate-language.md` §7 norm 4's case for the
agreement gate, from the other direction: the corruption was *upstream of the semantics*,

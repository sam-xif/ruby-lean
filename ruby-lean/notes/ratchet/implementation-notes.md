...
is a demand on the model (`RubyCore/`), not on this checker, and is recorded in `AGENTS.md` §Frontier
...
`RubyCore/Proof/KontFrame.lean`, 51 theorems, axiom-clean, ~430 lines, ~3 s to build.
...
`RubyCore/Proof/KontFrame.lean` is at **105 theorems**, axiom-clean. Since clink 52 it
...
`RubyCore/Proof/KontFrame.lean` is at **119 theorems**; the frame-pushing entries moved
...
change under `ruby-lean/` this clink, so the tier-0 difftest from clink 54 still stands.
...
ladder **42/83**. No change under `ruby-lean/` in clinks 55–56, so the tier-0 difftest from clink 54
still stands.
...
No change under `ruby-lean/` in clinks 55–57, so the tier-0 difftest from clink 54 still stands.
...
denotation gate's `#guard`s green; ladder **46/83**. No change under `ruby-lean/` since clink 54, so
...
tree carries none. This is the rot mode `ruby-lean/lakefile.toml`'s own comment predicts for a
...
under `RubyCore/Proof/`; `ruby-lean/Denote/{Apply,Ext,Local,Rules/Core,Rules/Lambda,Sem/Mut,

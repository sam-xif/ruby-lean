...
inductive Ty where
  ...
  /-- **The bottom type: an expression that does not produce a value.** Added at tier 6
      (`../notes/ratchet/implementation-notes.md` clink 5), where the recursion in `fun-recursive-factorial`
...
(`Denote/Sem/Core/State.lean`) rather than repeat a `match` inside every one of them.
...
a precision one, and `Denote/Sem/Core/State.lean`'s `sameAs_of_killAliasTy` is where it is spent. -/
...
check local to one entry, which is what `Denote/Ty/Local.lean`'s spine induction needs — it walks
...
(`Denote/Ty/Local.lean`'s spine case needs `σ = τ` as a *proposition*). Both kernel-reduce, which
...
def falsyTy : Ty → Ty
  ...
  -- `Ty.sameAs y ρ` denotes exactly `ρ`'s values (`Denote/Ty/Den.lean`), so answering `.never`

# AGENTS.md — the Sorbet-typed ladder (`Ratchet/`, `Semantics/`, `Denote/`)

Three of the four libraries of the `ruby-lean` Lake package; the fourth is the model
...
`Sem/ClassReady.lean` carries bounded class/dispatch edges, Object's cached eigenclass, and
the new objects' BasicObject ancestry through `CoreOk`; the existing boot gate checks them. `Ext` preserves edge bounds,
...
`Scope.closedIvars` distinguishes complete self fields from open instance annotations.
`InstanceEntry` derives the open self-spine and receiver type through a real method frame;
`InstanceSpineControls` refutes deriving completeness from an open instance, with a full-state
...
constructor payload/initialization, class publication, and class-rule admission remain.
`ClassRun` composes real fresh entry, a checked body contract, and frameK exit into a full
...
also rechecks old top-level bodies in the restored caller scope. No call re-infers a body from
...
boot-grounded full-old-contract omission witness: an empty declared child table may hide an
override of a globally reserved selector. Inherited lookup therefore needs owner-specific
absence, not just positive ancestor rows (§F39). Superclass creation and physical-chain facts
...
records. StateOk carries this bound through allocation, field/frame changes, class creation,
header publication and member installation. The existing member-freshness guard also checks
alias-safe owner separation; the boot bound is vacuous because no classes are declared yet.
...
`ClassGrowth` factors old-edge/walk transport from fresh-entry obligations. `SubclassReady`
preserves bounded edges, both fuel saturations, global reference liveness and ClassReady;
default-superclass readiness now specializes those generic proofs. Actual entry consumes
...
`initializerBodyCtxAt` now separates the allocated receiver class from initialize's lexical
owner using the existing Frame fields. `ConstructorState`/`ConstructorRun` prove entry,
annotated execution and caller restoration for that split; own constructors specialize the
same lemmas. `InheritedConstructor` derives first-owner lookup, code, allocation and new
...
[`notes/ratchet/implementation-notes.md`, clink 74](notes/ratchet/implementation-notes.md) for the witnesses and fixes.
...
*learnings* are load-bearing and are cited from live code and from `notes/ratchet/found-issues.md`: the
...
**Tiers 13–17 were then worked the same day**, in clinks 27–41 (`notes/ratchet/implementation-notes.md`):
...
**Clink 64 found and fixed a reachable soundness bug** (`notes/ratchet/found-issues.md` §F23), which is what
...
(`notes/ratchet/found-issues.md` §F1), and two of the three are the permanent negatives that pin the fix
...
ladder read 232/232 and 140/140 with the bug in place. See `notes/ratchet/found-issues.md` §F1 and
`notes/ratchet/implementation-notes.md` clink 46.
...
`Judge` threads an environment (`Judge Γ e τ Γ'`); see `notes/ratchet/implementation-notes.md` clink 2
...
(`ruby-lean/slice/slice-whole.rb`) — the rung *is* the measurement's input.
...
### Everything this pass found outside the checker is in [`notes/ratchet/found-issues.md`](notes/ratchet/found-issues.md)
...
Getting step 5 corrected the document's central claim (its own **§12**, `notes/ratchet/found-issues.md` §F21).
...
pairing. `notes/ratchet/implementation-notes.md`'s EMERGENCY EXIT (clink 64) is what that dead end looks
...
> `notes/ratchet/implementation-notes.md` §EMERGENCY EXIT.
...
> toplevel. Probing against CRuby (`notes/ratchet/found-issues.md` §A6a) showed that edge does not exist in
...
> layers, none smaller than a clink. Read `notes/ratchet/HANDOFF.md` first.
>
...
> `notes/ratchet/found-issues.md` **§F22** — `constAsgnOk`'s guard list is not the set of class names a `Ty` can
...
* **The three dispatch rungs** (clink 55, `Denote/Rules/Query.lean`, `CaseEq.lean`,
  ...
  **`SemJudge`'s first conjunct is now `Framed m m'`**, and that is the structural result of
  ...
  Both rules were **unsound as written** — `notes/ratchet/found-issues.md` **§F7**: their
...
* **`Judge.classOf`** (clink 56, `Denote/Rules/ClassOf.lean`) — `x.class`, and the rung that
  ...
  exact class object (`notes/ratchet/found-issues.md` §F8). The fix is §F12 and it is a change to the
...
* **`notes/ratchet/found-issues.md` §F18** (clink 59) — a *reachable* soundness bug found while sizing
...
* **`notes/ratchet/found-issues.md` §F19 — four reachable soundness bugs, one function** (clink 60; corpus
...
* **Superseded** (clink 57's framing of the same wall): the **fifteenth stall
  ...
  "a `noLocalAsgn` expression's run leaves the frame's locals alone" (`notes/ratchet/found-issues.md` §F13 —
...
`MissFree`'s absence was a **wrong answer**, not a gap — `notes/ratchet/found-issues.md` §F4, fixed in the
...
`notes/ratchet/found-issues.md` §F3). The
...
`notes/ratchet/found-issues.md` §A5/§F2).
...
* **`EnvOk`'s identity conjunct is `Value` equality, not `Value.identEq`.** The model's own
  ...
  `notes/ratchet/found-issues.md`; the denotation should not inherit a model quirk.
...
(`notes/ratchet/found-issues.md` §F1, the first entry in that file about the checker rather than about the
...
environment is the part that is not obvious and is argued for in `notes/ratchet/implementation-notes.md`
...
**`notes/ratchet/implementation-notes.md`** (clink 1) — that file is now the running log of
...
`Ratchet/` (the checker) imports nothing from `RubyCore/` — own `Expr`/`Ty`,
copied text, not linked; if the checker's copy and the real model's ever diverge,
that's a deliberate fork to notice and resolve, not a build error to silently paper
over. `Semantics/` is the one deliberate exception (§Semantics status): it imports the
real `RubyCore.Interp` directly, because hand-copying `stepFn`'s ~24k-line dependency
...
**How the isolation is enforced, since the merge.** The checker and the model used to be
two Lake packages (`ratchet/` requiring `lean/` by path), so `import RubyCore.…` under
`Ratchet/` was a build error. They are one package now — one `lakefile.toml`, one
`lean-toolchain`, one manifest, one `lake build` — which removes a whole class of
version-skew between the two and removes that refusal with it. So the refusal is a script:
`scripts/check-isolation.sh`, run as stage 0a of `scripts/run_typed_ratchet.sh`. It fails on
an `import RubyCore`/`Semantics`/`Denote` under `Ratchet/`, on the reverse import under
`RubyCore/`, and it reports (without failing) a second file under `Semantics/` importing the
model — the bridge is meant to stay one file.
...
- **`slice/*.rb`** + **`slice/drivers/*.rb`** + **`scripts/build_slice_rungs.py`** — tiers
  ...
  **committed**, so regenerating the corpus needs nothing; only *re-composing* needs the
  ...
  `cd ../difftest && PYTHONPATH=.. uv run python ../ruby-lean/scripts/build_slice_rungs.py`
...
3. `var`/`vasgn`/`seq` (6 rungs) — **all six climbed.** Real Ruby scoping (mutable
   ...
   `ArgumentError` (`notes/ratchet/implementation-notes.md` clink 2).
4. Conditionals (9 rungs) — **all nine at target** (`notes/ratchet/implementation-notes.md` clink 3).
...
5. Arrays/hashes (8 rungs) — **all eight at target** (`notes/ratchet/implementation-notes.md` clink 4).
...
6. Top-level functions (9 rungs) — **all nine at target** (`notes/ratchet/implementation-notes.md`
...
7. **Classes (16 rungs).** Real, varied class-based Ruby (construction, ivars,
   ...
   **All 16 climbed** (`notes/ratchet/implementation-notes.md` clinks 6 and 7). The object model:
...
8. **Modules (10 rungs).** **All ten climbed** (`notes/ratchet/implementation-notes.md` clink 8), and the
...
13. **Constants and scoped names (13 rungs). 12/13 — every rung it targets**
    (`notes/ratchet/implementation-notes.md` clinks 27–32). 928 `const` reads, 155 `cpath`s and 54 `casgn`s
...
regressions for `notes/ratchet/found-issues.md` §F3 and §F2/§A5 — both programs `validate` certified and both

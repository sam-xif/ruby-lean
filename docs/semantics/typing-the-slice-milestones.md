...
| route | content | state |
|---|---|---|
| **syntactic** | `framedProgB fp f p fuel = true → SemFrame … p fp` | **this is SF-T3**, now with a statement to induct toward; the definee lemma in `../../ruby-ruby-lean/notes/model/HANDOFF-SF.md` is its core |
...
| `M.new` is unsupported, not wrong | `harness/desugar-dt/bin/export-json` + `ruby-lean/.lake/build/bin/rubycore` |

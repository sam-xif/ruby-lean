...
| `concolic-dataflow.md` | **design (not built):** emitting *dataflow* (symbolic terms) from the model so the concolic engine needs no AST walk — the requirement (`derived.rb`: a condition is not a syntactic function of the inputs), why concrete traces can't recover it (value ambiguity), the design space (D0–D3), the recommended **observation-driven symbolic shadow machine** (no `stepFn` changes) that **self-checks against the concrete run**, staging S1–S4, and symbolic *dispatch* as the payoff |

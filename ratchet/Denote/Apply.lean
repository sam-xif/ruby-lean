...
/-!
# `Denote/Apply.lean` — calling a Proc value from inside a predicate
...
first-order type that is exactly right — `Denote/Den.lean`'s `denM_heap_only` proves the
...
* A **non-lambda** `return`/`break` inside the proc unwinds toward `cl.home`, a frame that
  ...
  over `.value` outcomes only (`Denote/Den.lean`).
...
`Denote/Den.lean`'s arrow denotation is stated at *one* machine: "called here, this proc
...
two are the `clos` side's probes, and they are here rather than in `Denote/Val.lean` because

...
theorem constructor_frame_state {κ : Ctx} {Γ : Env} {I : Ty} {m : Machine}
    ...
    InitState m.heap (initializerBodyCtx κ cn) ps .ivar0 (constructorFrame m k md ps args) := by
  have he := ctorAllocated_ext hm hc
  have hs := instance_enter_state (ctorAllocated_state hm hc) ht ha (site.ext he hc.live) hp code rfl

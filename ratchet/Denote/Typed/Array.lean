...
theorem stepSpec_array {Γ : Env} {m : Machine} {τ : Ty}
    (hm : StateOk ctx0 Γ .ivar0 m) (hk : m.kont = [])
    (xs : List Value) (hd : ∀ x ∈ xs, denM τ m x) :
    StepSpec m Γ (.arrayOf τ) (Interp.continueArray m xs []) := by
  ...
  have h := RunSpec.answer (a := .val (.ref m.heap.objs.size))
    (show ResultOk m Γ (.arrayOf τ) _ n from
...
private theorem array_spec {Γ Γ' : Env} {es : List Ratchet.Expr} {tys : List Ty}
    (hs : SemAllA Γ es tys Γ') {τ : Ty} (hf : FirstOrder τ = true)
    (ht : ∀ σ ∈ tys, ∀ m v, denM σ m v → denM τ m v)
    {m : Machine} (hm : StateOk ctx0 Γ .ivar0 m) (hk : m.kont = [])
    (acc : List Value) (ha : ∀ v ∈ acc, denM τ m v) :
    StepSpec m Γ' (.arrayOf τ) (Interp.continueArray m acc (toRubyList es)) := by
  induction hs generalizing m acc with
  | nil => exact stepSpec_array hm hk acc ha
  | @cons Γ Γ₁ Γ₂ e es σ tys he hs hp ih =>
    rw [continueArray_cons m acc e es hp]
    simp only [StepSpec, Interp.withKont, hk]
    change RunSpec m (pushK [.arrK acc (toRubyList es)] (evalFrom m e)) Γ₂ (.arrayOf τ)
    apply RunSpec.bind he hm (by
      intro k h tag
    ...
    | val v =>
      ...
      have h : RunSpec (deliverA (.val v) n [])
          (deliverA (.val v) n [.arrK acc (toRubyList es)]) Γ₂ (.arrayOf τ) := by
...
theorem SemA.arrayLit {Γ Γ' : Env} {es : List Ratchet.Expr} {tys : List Ty}
    ...
    SemSafeA Γ (.array es) (.arrayOf (elemTy tys)) Γ' := by

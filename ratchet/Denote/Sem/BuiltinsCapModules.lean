import Denote.Sem.BuiltinsCapRegex

/-!
# `Denote/Sem/BuiltinsCapModules.lean` — `Builtins.runModules`, for the heap half
...
theorem runModules_cap (bid : String) (recv : Value) (args : List Value) (m : Machine) :
    ∀ v m', Builtins.runModules bid recv args m = .ok v m' → MCap m m' := by
  intro v m' h
  rw [Builtins.runModules.eq_def] at h
  dsimp only at h
  cap_arms_mod

#print axioms runModules_cap

import Denote.Sem.BuiltinsCapModules

/-!
# `Denote/Sem/BuiltinsCapCollections.lean` — `Builtins.runCollections`, for the heap half
...
theorem runCollections_cap (bid : String) (recv : Value) (args : List Value) (m : Machine) :
    ∀ v m', Builtins.runCollections bid recv args m = .ok v m' → MCap m m' := by
  intro v m' h
  rw [Builtins.runCollections.eq_def] at h
  dsimp only at h
  cap_arms

#print axioms runCollections_cap

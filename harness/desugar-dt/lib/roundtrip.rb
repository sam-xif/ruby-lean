...
module Roundtrip
  Result = Struct.new(
    ...
    :reason, :coverage, :core, :rendered, :obs_src, :obs_core, :normal_form,
    keyword_init: true
  )
  ...
  def check(src, inject_bug: false)
    ...
    # 3. pure check: normal-form / idempotence (warning, not a disagreement — C9)
    normal_form =
      begin
    ...
    Result.new(status: :disagree, bucket: bucket_for(obs_src, obs_core), coverage: coverage,
               ...
               normal_form: normal_form, reason: diff(obs_src, obs_core))
  end

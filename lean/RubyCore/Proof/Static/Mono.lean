...
theorem infer_table_ret : ∀ (D : Decls) (Γ : Env) (e : Expr) (top : Bool) (ctx : FrameCtx),
  ...
  | case92 D Γ t top ctx e' τt Γt Dt τe Γe De hE hT hagree ih2 ih1 =>
...
theorem infer_mono_all : ∀ (D : Decls) (Γ : Env) (e : Expr) (top : Bool) (ctx : FrameCtx),
  ...
  | case92 D Γ t top ctx e' τt Γt Dt τe Γe De hE hT hagree ihT ihE =>
    intro F' hs hdf τ Γ' D₀ h
  ...
  | case95 D Γ t top ctx τt Γt Dt hT hcond ihT =>
    intro F' hs hdf τ Γ' D₀ h
  ...
  | case103 D Γ top ctx e rest τe Γ₁ D₁ he τs Γ₂ D₂ hrest ihE ihR =>
    intro F' hs hdf τs' Γ' D₀ h
    simp only [defFreeAll, Bool.and_eq_true] at hdf
    obtain ⟨rfl, hmE⟩ := ihE F' hs (by simp_all [defFreeAll]) _ _ _ he
  ...
  | _ =>
    ...
    first
      ...
      -- transport is three `split`s and a recomputation. An IH-free case, like `case98`.
      | (revert h
         simp only [infer]
         split
         · rename_i htop
           ...
           · rename_i hil
             ...
             · rename_i hse
               ...
               simp only [Option.some.injEq, Prod.mk.injEq] at hq
               obtain ⟨rfl, rfl, rfl⟩ := hq
               exact ⟨rfl, by simp only [infer, htop, hil, hse, if_true, reduceIte]⟩
             · simp
           · simp
         · simp)

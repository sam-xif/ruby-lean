...
theorem step_ok {D : Decls} {m : Machine} (h : Inv D m) : StepOk D (stepFn m) := by
  obtain ⟨htab, hhook, hsat, hstr, Γ, Γs, hfs, hc⟩ := h
  ...
  · -- ## control = eval e
    ...
    case int n =>
      ...
      obtain ⟨rfl, rfl⟩ := hinf
      exact inv_value hfs htab hhook hsat hstr rfl hk
    case tru =>
      simp only [infer, Option.some.injEq, Prod.mk.injEq] at hinf
      obtain ⟨rfl, rfl⟩ := hinf
      exact inv_value hfs htab hhook hsat hstr rfl hk
    ...
    case str s =>
      simp only [infer, Option.some.injEq, Prod.mk.injEq] at hinf
      obtain ⟨rfl, rfl⟩ := hinf
      exact inv_grow_value hfs htab hhook hsat hstr
    ...
    case var k x =>
      ...
      case lvar =>
        ...
        obtain ⟨rfl, rfl⟩ := heq
        exact inv_value hfs htab hhook hsat hstr (hl x _ hg) hk
    ...
    case vasgn k x rhs =>
      ...
      case lvar =>
        ...
        · rename_i σ Γ₁ hrhs
          ...
          obtain ⟨rfl, rfl⟩ := hinf
          exact inv_push hfs htab hhook hsat hstr hrhs (KontOk.asgn hk)
    ...
    case seq es =>
      ...
      | cons e₁ rest =>
        ...
        | nil =>
          ...
          exact inv_eval hfs htab hhook hsat hstr hinf hk
        | cons e₂ rest' =>
          simp only [inferSeq] at hinf
          split at hinf
          · rename_i σ Γ₁ h₁
            exact inv_push hfs htab hhook hsat hstr h₁ (KontOk.seqCons hinf hk)
          · exact absurd hinf (by simp)
    case if' c t els =>
      simp only [infer] at hinf
      split at hinf
      · rename_i σ Γ₁ hcnd
        exact inv_push hfs htab hhook hsat hstr hcnd (KontOk.ifK hinf hk)
      · exact absurd hinf (by simp)
    case while' c body =>
      simp only [infer] at hinf
      split at hinf
        ...
        · rename_i hΓ₁
          ...
          · rename_i σb Γ₂ hbody
            ...
            · rename_i hΓ₂
              ...
              simp only [Option.some.injEq, Prod.mk.injEq] at hinf
              obtain ⟨rfl, rfl⟩ := hinf
              exact inv_push hfs htab hhook hsat hstr hcnd
    ...
    case def' name params body =>
      ...
      have hres : ∀ (m₀ : Machine),
        ...
        refine ⟨ht', hh', hsat', hstr', Γ', Γs, ?_, ?_⟩
        ...
        | cons arg extra =>
          ...
          | nil =>
            ...
            | none =>
              ...
              simp only [evalExpr]
              cases r <;>
                try exact inv_push hfs htab hhook hsat hstr hr (KontOk.recvK hsg ha hk)
  ...
  · -- ## control = value v
    ...
    | nil => trivial
    | @seqNil Γ Γs τ k hk' =>
      exact inv_value hfs htab hhook hsat hstr hv hk'
    | @seqCons Γ Γs τ e₁ es τ' Γ' k hseq hk' =>
      cases es with
      | nil =>
        simp only [inferSeq] at hseq
        exact inv_push hfs htab hhook hsat hstr hseq (KontOk.seqNil hk')
      | cons e₂ es' =>
        simp only [inferSeq] at hseq
        split at hseq
        · rename_i σ Γ₁ h₁
          exact inv_push hfs htab hhook hsat hstr h₁ (KontOk.seqCons hseq hk')
    ...
    | @asgn Γ Γs τ x k hk' =>
      ...
      refine ⟨htab, hhook, hsat, hstr, envSet Γ x τ, Γs, ?_, ⟨τ, hv, hk'⟩⟩
      ...
      | some e₂ =>
        ...
        · rename_i τt Γt τe Γe ht he
          ...
          · rename_i hagree
            ...
            · simp only [hb, if_true]; exact inv_eval hfs htab hhook hsat hstr ht hk'
            · simp only [hb]; exact inv_eval hfs htab hhook hsat hstr he hk'
      ...
      | none =>
        ...
        · rename_i τt Γt ht
          ...
          · rename_i hnil
            ...
            by_cases hb : v.truthy
            · simp only [hb, if_true]; exact inv_eval hfs htab hhook hsat hstr ht hk'
            · simp only [hb]; exact inv_value hfs htab hhook hsat hstr rfl hk'
          · exact absurd hif (by simp)
        · exact absurd hif (by simp)
    | @whileCond Γ Γs τ c body k hloop hk' =>
      ...
      · simp only [hb, if_true]
        exact inv_push hfs htab hhook hsat hstr hbody (KontOk.whileBody ⟨⟨σc, hcnd⟩, ⟨σb, hbody⟩⟩ hk')
      · simp only [hb]
        exact inv_value hfs htab hhook hsat hstr rfl hk'
    | @whileBody Γ Γs τ c body k hloop hk' =>
      obtain ⟨⟨σc, hcnd⟩, ⟨σb, hbody⟩⟩ := hloop
      exact inv_push hfs htab hhook hsat hstr hcnd (KontOk.whileCond ⟨⟨σc, hcnd⟩, ⟨σb, hbody⟩⟩ hk')
    | @frameK Γ Γ' Γs τ fid k hk' =>
      ...
      exact ⟨htab, hhook, hsat, hstr, Γ', Γs, hfs.tail, ⟨τ, hv, hk'⟩⟩

...
theorem judge_mono {A : SemAxioms} {D : Decls} {Γ : Env} {e : Expr} {top : Bool}
  ...
  case hsemantic =>
    intro D Γ top ctx cl hmem hff hreq hqm hfr hfn hdisc hmeth2 _ _ _ D2 _
    have hrows : cl.rows = [] ∧ cl.freshNames = [] := by
      rcases hdisc with h | h
      · exact h
      · rw [h] at hmeth2; exact Bool.noConfusion hmeth2
    refine ⟨by simp [hrows.1, addRows], ?_⟩
    have hsem : Judge A D2 Γ cl.e top ctx cl.τ Γ (addRows D2 cl.rows) :=
      .semantic hmem hff hreq hqm (by rw [hrows.1]; simp) (by rw [hrows.2]; simp)
        (Or.inl hrows)
    simp only [hrows.1, addRows, List.foldl] at hsem
    exact hsem
  -- JudgeRecv
  case _ =>  -- self
  ...
  case _ =>  -- single
    ...
    · -- a claimed element: rebuild via the semantic leaf, canonical by the coupling
      ...
      refine ⟨by simp [hrows.1, addRows], ?_⟩
      have hsem : Judge A D2 Γ' cl.e top ctx cl.τ Γ' (addRows D2 cl.rows) :=
        .semantic hclA hfe hreq hqm (by rw [hrows.1]; simp) (by rw [hrows.2]; simp)
          (Or.inl hrows)
      simp only [hrows.1, addRows, List.foldl] at hsem
      exact .single hsem
        (hcpl := fun _ => ⟨cl, hclA, rfl, rfl, rfl, by simp [hrows.1, addRows],
          Or.inl hrows, hreq, hqm, by rw [hrows.1]; simp, by rw [hrows.2]; simp⟩)
  case _ =>  -- cons
    ...
    · replace hfe : fragHead e = false := by simpa using hfe
      ...
      have hrows : cl.rows = [] ∧ cl.freshNames = [] := by
        ...
        · exact h
        · rw [h] at hmeth2; exact Bool.noConfusion hmeth2
      simp only [hrows.1, addRows, List.foldl] at hD₁
      have hD₁' : D₁ = D := hD₁
      subst hD₁'
      obtain ⟨rfl, hr'⟩ := ihr hmeth2 (fun e' he' => hm e' (by simp [he']))
        (by simp only [defFreeAll, Bool.and_eq_true]; exact hdf.2) hs

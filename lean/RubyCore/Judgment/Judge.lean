...
`Judge D Γ e top ctx τ Γ' D'` — under declaration table `D`, local environment `Γ`,
...
/-- An optional subterm: judged if present; `nil` and the incoming environment and
    table if absent. -/
inductive JudgeOpt : Decls → Env → Option Expr → Bool → JCtx → Ty → Env → Decls → Prop where
  | none {D Γ top ctx} : JudgeOpt D Γ none top ctx .nilT Γ D
  | some {D Γ e top ctx τ Γ' D'} :
      Judge D Γ e top ctx τ Γ' D' → JudgeOpt D Γ (some e) top ctx τ Γ' D'
...
inductive JudgeRecv : Decls → Env → Option Expr → Bool → JCtx → Ty → Env → Decls → Prop where
  | self {D Γ top ctx cc} :
      ctx.selfCls = some cc → JudgeRecv D Γ none top ctx (.cls cc) Γ D
  | expl {D Γ r top ctx τ Γ' D'} :
      Judge D Γ r top ctx τ Γ' D' → JudgeRecv D Γ (some r) top ctx τ Γ' D'
...
inductive JudgeSeq : Decls → Env → List Expr → Bool → JCtx → Ty → Env → Decls → Prop where
  | nil {D Γ top ctx} : JudgeSeq D Γ [] top ctx .nilT Γ D
  | single {D Γ e top ctx τ Γ' D'} :
      Judge D Γ e top ctx τ Γ' D' → JudgeSeq D Γ [e] top ctx τ Γ' D'
  | cons {D Γ e e₂ rest top ctx τ₁ Γ₁ D₁ τ Γ' D'} :
      Judge D Γ e top ctx τ₁ Γ₁ D₁ →
      JudgeSeq D₁ Γ₁ (e₂ :: rest) top ctx τ Γ' D' →
      JudgeSeq D Γ (e :: e₂ :: rest) top ctx τ Γ' D'

/-- An argument list: the same threading, types kept in order (matched against a
    signature's parameter list). -/
inductive JudgeArgs : Decls → Env → List Expr → Bool → JCtx → List Ty → Env → Decls → Prop where
  | nil {D Γ top ctx} : JudgeArgs D Γ [] top ctx [] Γ D
  | cons {D Γ e rest top ctx τ Γ₁ D₁ τs Γ' D'} :
      Judge D Γ e top ctx τ Γ₁ D₁ →
      JudgeArgs D₁ Γ₁ rest top ctx τs Γ' D' →
      JudgeArgs D Γ (e :: rest) top ctx (τ :: τs) Γ' D'
...
/-- The elements of an array literal: element types erased. A `.splat` element goes
    ...
    *is* an Array). -/
inductive JudgeElems : Decls → Env → List Expr → Bool → JCtx → Env → Decls → Prop where
  | nil {D Γ top ctx} : JudgeElems D Γ [] top ctx Γ D
  | cons {D Γ e rest top ctx τ Γ₁ D₁ Γ' D'} :
      Judge D Γ e top ctx τ Γ₁ D₁ →
      JudgeElems D₁ Γ₁ rest top ctx Γ' D' →
      JudgeElems D Γ (e :: rest) top ctx Γ' D'

/-- The pairs of a hash literal — key then value, left to right. -/
inductive JudgePairs : Decls → Env → List (Expr × Expr) → Bool → JCtx → Env → Decls → Prop where
  | nil {D Γ top ctx} : JudgePairs D Γ [] top ctx Γ D
  | cons {D Γ k v rest top ctx τk Γ₁ D₁ τv Γ₂ D₂ Γ' D'} :
      Judge D Γ k top ctx τk Γ₁ D₁ →
      Judge D₁ Γ₁ v top ctx τv Γ₂ D₂ →
      JudgePairs D₂ Γ₂ rest top ctx Γ' D' →
      JudgePairs D Γ ((k, v) :: rest) top ctx Γ' D'

/-- The entries of a brace-less keyword-argument marker: each value is judged; a
    `.dyn` entry's key is too. -/
inductive JudgeKwEntries : Decls → Env → List KwEntry → Bool → JCtx → Env → Decls → Prop where
  | nil {D Γ top ctx} : JudgeKwEntries D Γ [] top ctx Γ D
  | pair {D Γ k v rest top ctx τ Γ₁ D₁ Γ' D'} :
      Judge D Γ v top ctx τ Γ₁ D₁ →
      JudgeKwEntries D₁ Γ₁ rest top ctx Γ' D' →
      JudgeKwEntries D Γ (.pair k v :: rest) top ctx Γ' D'
  | splat {D Γ v rest top ctx τ Γ₁ D₁ Γ' D'} :
      Judge D Γ v top ctx τ Γ₁ D₁ →
      JudgeKwEntries D₁ Γ₁ rest top ctx Γ' D' →
      JudgeKwEntries D Γ (.splat v :: rest) top ctx Γ' D'
  | dyn {D Γ k v rest top ctx τk Γ₁ D₁ τv Γ₂ D₂ Γ' D'} :
      Judge D Γ k top ctx τk Γ₁ D₁ →
      Judge D₁ Γ₁ v top ctx τv Γ₂ D₂ →
      JudgeKwEntries D₂ Γ₂ rest top ctx Γ' D' →
      JudgeKwEntries D Γ (.dyn k v :: rest) top ctx Γ' D'
...
inductive JudgeRescues : Decls → Env → Bool → JCtx →
    List (List Expr × Option (TargetKind × String) × Expr) → List Ty → Prop where
  | nil {D Γ top ctx} : JudgeRescues D Γ top ctx [] []
  | cons {D Γ top ctx excs tgt hbody rest τs Γx τx Γh τh Γh' τrs} :
      JudgeArgs D Γ excs top ctx τs Γx D →
      τx = (match τs with | [.clsOf n] => .cls n | _ => .any) →
      Γh = (match tgt with | some (_, x) => envSet Γ x τx | none => Γ) →
      Judge D Γh hbody top (rescueCtx ctx) τh Γh' D →
      JudgeRescues D Γ top ctx rest τrs →
      JudgeRescues D Γ top ctx ((excs, tgt, hbody) :: rest) (τh :: τrs)
...
/-- The `else` clause of a `begin` region: its type **replaces** the body's where
    ...
    region's). -/
inductive JudgeElse : Decls → Env → Option Expr → Bool → JCtx → Ty → Ty → Prop where
  | none {D Γ top ctx τb} : JudgeElse D Γ none top ctx τb τb
  | some {D Γ el top ctx τb τl Γl' } :
      Judge D Γ el top ctx τl Γl' D → JudgeElse D Γ (some el) top ctx τb τl

/-- The `ensure` clause: judged for well-typedness, value discarded, table pinned. -/
inductive JudgeEns : Decls → Env → Option Expr → Bool → JCtx → Prop where
  | none {D Γ top ctx} : JudgeEns D Γ none top ctx
  | some {D Γ en top ctx τ Γ'} :
      Judge D Γ en top ctx τ Γ' D → JudgeEns D Γ (some en) top ctx

/-- **The judgment.** `Judge D Γ e top ctx τ Γ' D'` — see the module docstring for
    ...
    in the reading. -/
inductive Judge : Decls → Env → Expr → Bool → JCtx → Ty → Env → Decls → Prop where
  -- ## Literals — derived, each one `evalExpr`'s own answer.
  | int {D Γ n top ctx} : Judge D Γ (.int n) top ctx .int Γ D
  | flt {D Γ x top ctx} : Judge D Γ (.flt x) top ctx .float Γ D
  | str {D Γ s top ctx} : Judge D Γ (.str s) top ctx (.cls "String") Γ D
  | sym {D Γ s top ctx} : Judge D Γ (.sym s) top ctx .sym Γ D
  | tru {D Γ top ctx} : Judge D Γ .tru top ctx .bool Γ D
  | fls {D Γ top ctx} : Judge D Γ .fls top ctx .bool Γ D
  | nil {D Γ top ctx} : Judge D Γ .nil top ctx .nilT Γ D
  ...
  | self {D Γ top ctx cc} :
      ctx.selfCls = some cc → Judge D Γ .self' top ctx (.cls cc) Γ D
  -- ## Variable reads.
  | varLvar {D Γ x top ctx τ} :
      envGet? Γ x = some τ → Judge D Γ (.var .lvar x) top ctx τ Γ D
  ...
  | varIvar {D Γ x top ctx cc σ} :
      ...
      Judge D Γ (.var .ivar x) top ctx (mkNilable σ) Γ D
  | varGvar {D Γ x top ctx σ} :
      plainGlobal x = true → globalTy? D x = some σ →
      Judge D Γ (.var .gvar x) top ctx (mkNilable σ) Γ D
  ...
  | varCvar {D Γ x top ctx} (τ : Ty) :
      Judge D Γ (.var .cvar x) top ctx (mkNilable τ) Γ D
  ...
  | vasgnLvar {D Γ x rhs top ctx τ Γ₁ D₁} :
      ctx.inBlock = false →
      Judge D Γ rhs top ctx τ Γ₁ D₁ →
      Judge D Γ (.vasgn .lvar x rhs) top ctx τ (envSet Γ₁ x τ) D₁
  | vasgnIvarDecl {D Γ x rhs top ctx cc τ Γ₁ D₁ σ} :
      ctx.selfCls = some cc →
      Judge D Γ rhs top ctx τ Γ₁ D₁ →
      ivarTy? D₁ cc x = some σ → SubJ τ σ →
      Judge D Γ (.vasgn .ivar x rhs) top ctx τ Γ₁ D₁
  ...
  | casgn {D Γ nm rhs top ctx τ Γ₁ D₁} :
      Judge D Γ rhs top ctx τ Γ₁ D₁ →
      Judge D Γ (.casgn nm rhs) top ctx τ Γ₁ D₁
  | cpathAsgn {D Γ base nm rhs top ctx τb Γ₁ D₁ τ Γ₂ D₂} :
      JudgeOpt D Γ base top ctx τb Γ₁ D₁ →
      Judge D₁ Γ₁ rhs top ctx τ Γ₂ D₂ →
      Judge D Γ (.cpathAsgn base nm rhs) top ctx τ Γ₂ D₂
  -- ## Sends.
  ...
  | send {D Γ recvO mname args top ctx τr Γ₁ D₁ τs Γ₂ D₂ ps τret} :
      JudgeRecv D Γ recvO top ctx τr Γ₁ D₁ →
      JudgeArgs D₁ Γ₁ args top ctx τs Γ₂ D₂ →
      sigOf D₂ τr mname = some (ps, τret) → SubJs τs ps →
      Judge D Γ (.send recvO mname args none) top ctx τret Γ₂ D₂
  ...
  | sendIter0 {D Γ r mname ps ls body top ctx τr Γ₁ D₁ x σp βret τret τb Γb'} :
      ...
      Judge D₁ ((x, σp) :: anyEnv Γ₁) body false (jBlockCtx ctx) τb Γb' D₁ →
      SubJ τb βret →
      SubEnv ((x, σp) :: anyEnv Γ₁) Γb' →
      ctx.inBlock = false →
      Judge D Γ (.send (some r) mname [] (some (.block ps ls body))) top ctx τret Γ₁ D₁
  ...
  | sendIterA {D Γ recvO mname a as ps ls body top ctx τr Γ₁ τs Γ₂ x σp βret dps τret τb Γb'} :
      JudgeRecv D Γ recvO top ctx τr Γ₁ D →
      JudgeArgs D Γ₁ (a :: as) top ctx τs Γ₂ D →
  ...
  | sendBlockpass {D Γ recvO mname args bo top ctx τr Γ₁ D₁ τs Γ₂ D₂ τp Γ₃ D₃ ps τret} :
      JudgeRecv D Γ recvO top ctx τr Γ₁ D₁ →
      JudgeArgs D₁ Γ₁ args top ctx τs Γ₂ D₂ →
      JudgeOpt D₂ Γ₂ bo top ctx τp Γ₃ D₃ →
      sigOf D₃ τr mname = some (ps, τret) → SubJs τs ps →
      Judge D Γ (.send recvO mname args (some (.blockpass bo))) top ctx τret Γ₃ D₃
  ...
  | vcall {D Γ mname top ctx cc τret} :
      ...
      Judge D Γ (.vcall mname) top ctx τret Γ D
  -- ## Argument-position markers.
  | kwargs {D Γ entries top ctx Γ' D'} :
      JudgeKwEntries D Γ entries top ctx Γ' D' →
      Judge D Γ (.kwargs entries) top ctx (.cls "Hash") Γ' D'
  ...
  -- what refuses it.
  | fwd {D Γ top ctx} : Judge D Γ .fwd top ctx .any Γ D
  | splatAnon {D Γ top ctx} : Judge D Γ (.splat none) top ctx (.cls "Array") Γ D
  | splatArray {D Γ o top ctx Γ₁ D₁} :
      Judge D Γ o top ctx (.cls "Array") Γ₁ D₁ →
      Judge D Γ (.splat (some o)) top ctx (.cls "Array") Γ₁ D₁
  | splatArrayOf {D Γ o top ctx τ Γ₁ D₁} :
      Judge D Γ o top ctx (.arrayOf τ) Γ₁ D₁ →
      Judge D Γ (.splat (some o)) top ctx (.arrayOf τ) Γ₁ D₁
  ...
  | yield' {D Γ args top ctx τs Γ' D' bs} :
      ...
      Judge D Γ (.yield' args) top ctx bs.ret Γ' D'
  -- ## Control flow.
  --
  ...
  | ifElse {D Γ cond t el top ctx τc Γ₁ D₁ τt Γt Dt τe Γe τj Γc} :
      Judge D Γ cond top ctx τc Γ₁ D₁ →
      Judge D₁ Γ₁ t top ctx τt Γt Dt →
      Judge D₁ Γ₁ el top ctx τe Γe Dt →
      SubJ τt τj → SubJ τe τj →
      SubEnv Γc Γt → SubEnv Γc Γe →
      Judge D Γ (.if' cond t (some el)) top ctx τj Γc Dt
  | ifNone {D Γ cond t top ctx τc Γ₁ D₁ τt Γt τj Γc} :
      Judge D Γ cond top ctx τc Γ₁ D₁ →
      Judge D₁ Γ₁ t top ctx τt Γt D₁ →
      SubJ τt τj → SubJ .nilT τj →
      SubEnv Γc Γt → SubEnv Γc Γ₁ →
      Judge D Γ (.if' cond t none) top ctx τj Γc D₁
  ...
  | ifNarrowElse {D Γ x t el top ctx τ₀ τt Γt Dt τe Γe τj Γc} :
      envGet? Γ x = some τ₀ →
      Judge D (envSet Γ x (dropNil τ₀)) t top ctx τt Γt Dt →
      Judge D (envSet Γ x (elseNarrow τ₀)) el top ctx τe Γe Dt →
      SubJ τt τj → SubJ τe τj →
      SubEnv Γc Γt → SubEnv Γc Γe →
      Judge D Γ (.if' (.var .lvar x) t (some el)) top ctx τj Γc Dt
  | ifNarrowNone {D Γ x t top ctx τ₀ τt Γt τj Γc} :
      envGet? Γ x = some τ₀ →
      Judge D (envSet Γ x (dropNil τ₀)) t top ctx τt Γt D →
      SubJ τt τj → SubJ .nilT τj →
      SubEnv Γc Γt → SubEnv Γc Γ →
      Judge D Γ (.if' (.var .lvar x) t none) top ctx τj Γc D
  ...
  | while' {D Γ Γl cond body top ctx τc Γ₁ τb Γ₂} :
      SubEnv Γl Γ →
      Judge D Γl cond top (loopCtx ctx Γl) τc Γ₁ D →
      SubEnv Γl Γ₁ →
      Judge D Γl body top (loopCtx ctx Γl) τb Γ₂ D →
      SubEnv Γl Γ₂ →
      Judge D Γ (.while' cond body) top ctx .nilT Γl D
  | dowhile {D Γ Γl body cond top ctx τb Γ₁ τc Γ₂} :
      ...
      Judge D Γl cond top (loopCtx ctx Γl) τc Γ₂ D →
      SubEnv Γl Γ₂ →
      Judge D Γ (.dowhile body cond) top ctx .nilT Γl D
  ...
  | for' {D Γ tgts coll body top ctx τc Γ₁ D₁ τe Γb τb Γ₂} :
      Judge D Γ coll top ctx τc Γ₁ D₁ →
      τe = (match τc with | .arrayOf τ => τ | _ => .any) →
      Γb = bindTargetsJ tgts τe Γ₁ →
      Judge D₁ Γb body top (loopCtx ctx Γb) τb Γ₂ D₁ →
      SubEnv Γb Γ₂ →
      Judge D Γ (.for' tgts coll body) top ctx τc Γb D₁
  -- ## Jumps — each sound only where its target exists, per the context channels.
  | retSome {D Γ e' top ctx σ τ Γ₁ D₁} :
      ctx.ret = some σ →
      Judge D Γ e' top ctx τ Γ₁ D₁ → SubJ τ σ →
      Judge D Γ (.ret (some e')) top ctx .nilT Γ₁ D₁
  | retNil {D Γ top ctx σ} :
      ctx.ret = some σ → SubJ .nilT σ →
      Judge D Γ (.ret none) top ctx .nilT Γ D
  ...
  | nxtNil {D Γ top ctx Γl} :
      ...
      Judge D Γ (.nxt none) top ctx .nilT Γ D
  | nxtSome {D Γ e' top ctx Γl τ Γ₁} :
      top = false → ctx.inLoop = some Γl →
      Judge D Γ e' top ctx τ Γ₁ D → SubEnv Γl Γ₁ →
      Judge D Γ (.nxt (some e')) top ctx .nilT Γ₁ D
  ...
  | brkNil {D Γ top ctx Γl} :
      top = false → ctx.inLoop = some Γl →
      Judge D Γ (.brk none) top ctx .nilT Γ D
  | brkSome {D Γ e' top ctx Γl τ Γ₁ D₁} :
      top = false → ctx.inLoop = some Γl →
      Judge D Γ e' top ctx τ Γ₁ D₁ → SubJ τ .nilT →
      Judge D Γ (.brk (some e')) top ctx .nilT Γ₁ D₁
  -- `retry` restarts a `rescue`'s region — sound here via the `inRescue` channel
  -- (`chk` can only claim it; J2).
  | retry' {D Γ top ctx} :
      ctx.inRescue = true → Judge D Γ .retry' top ctx .nilT Γ D
  ...
  | redo' {D Γ top ctx Γl} :
      ...
      Judge D Γ .redo' top ctx .nilT Γ D
  -- ## Definition forms.
  --
  ...
  | defDecl {D Γ name ps body top ctx τs σ bs τb Γb' } :
      ...
      τs.length = ps.length →
      Judge D (bindParamsJ ps τs []) body false (methodCtx ctx name τs (some σ) bs) τb Γb' D →
      SubJ τb σ →
      Judge D Γ (.def' name ps body) top ctx .sym Γ D
  ...
  | defPromote {D Γ name body top ctx τb Γb'} :
      declaresName D name = false → name ≠ "method_added" →
      Judge D [] body false (methodCtx ctx name [] none none) τb Γb' D →
      ...
      Judge D Γ (.def' name [] body) top ctx .sym Γ
  ...
  | defs {D Γ recv name ps body top ctx τrcv Γ₁ D₁ τs σ bs τb Γb'} :
      Judge D Γ recv top ctx τrcv Γ₁ D₁ →
      τs.length = ps.length →
      Judge D₁ (bindParamsJ ps τs []) body false (singletonCtx ctx name τs σ bs) τb Γb' D₁ →
      Judge D Γ (.defs recv name ps body) top ctx .sym Γ₁ D₁
  ...
  | classTop {D Γ name body ctx τ Γb' Db} :
      ...
      Judge D [] body false ({ cls := name } : JCtx) τ Γb' Db →
      Judge D Γ (.class' name none body) true ctx τ Γ Db
  ...
  | classSup {D Γ name s body top ctx τs' Γ₁ D₁ τ Γb' Db'} :
      Judge D Γ s top ctx τs' Γ₁ D₁ →
      Judge D₁ [] body false ({ cls := name } : JCtx) τ Γb' Db' →
      Judge D Γ (.class' name (some s) body) top ctx τ Γ₁ D₁
  | module' {D Γ name body top ctx τ Γb' Db'} :
      Judge D [] body false ({ cls := name } : JCtx) τ Γb' Db' →
      Judge D Γ (.module' name body) top ctx τ Γ D
  | scopedClass {D Γ base name body top ctx τb' Γ₁ D₁ τ Γb' Db'} :
      JudgeOpt D Γ base top ctx τb' Γ₁ D₁ →
      Judge D₁ [] body false ({ cls := name } : JCtx) τ Γb' Db' →
      Judge D Γ (.scopedClass base name body) top ctx τ Γ₁ D₁
  | scopedModule {D Γ base name body top ctx τb' Γ₁ D₁ τ Γb' Db'} :
      JudgeOpt D Γ base top ctx τb' Γ₁ D₁ →
      Judge D₁ [] body false ({ cls := name } : JCtx) τ Γb' Db' →
      Judge D Γ (.scopedModule base name body) top ctx τ Γ₁ D₁
  ...
  | sclass {D Γ obj body top ctx τo Γ₁ D₁ nm τ Γb' Db'} :
      Judge D Γ obj top ctx τo Γ₁ D₁ →
      nm = (match τo with | .clsOf n => n | _ => ctx.cls) →
      Judge D₁ [] body false ({ cls := nm } : JCtx) τ Γb' Db' →
      Judge D Γ (.sclass obj body) top ctx τ Γ₁ D₁
  ...
  | begin' {D Γ body rescues els ens top ctx τb Γb₂ τrs τe τj} :
      Judge D Γ body top ctx τb Γb₂ D →
      JudgeRescues D Γ top ctx rescues τrs →
      JudgeElse D Γ els top ctx τb τe →
      JudgeEns D Γ ens top ctx →
      (∀ τ ∈ τe :: τrs, SubJ τ τj) →
      Judge D Γ (.begin' body rescues els ens) top ctx τj Γ D
  ...
  | super' {D Γ args top ctx mn τs Γ₁ D₁ d} :
      ...
      Judge D Γ (.super' args none) top ctx d.ret Γ₁ D₁
  ...
  | zsuper {D Γ top ctx mn ps d} :
      ...
      Judge D Γ (.zsuper none) top ctx d.ret Γ D
  ...
  | alias' {D Γ newName old top ctx} :
      ...
      Judge D Γ (.alias' newName old) top ctx .nilT Γ D
  ...
  | defined {D Γ e top ctx} :
      Judge D Γ (.defined e) top ctx (.nilable (.cls "String")) Γ D
  -- ## Composites.
  | array {D Γ es top ctx Γ' D'} :
      JudgeElems D Γ es top ctx Γ' D' →
      Judge D Γ (.array es) top ctx (.cls "Array") Γ' D'
  | hash {D Γ pairs top ctx Γ' D'} :
      JudgePairs D Γ pairs top ctx Γ' D' →
      Judge D Γ (.hash pairs) top ctx (.cls "Hash") Γ' D'
  | seq {D Γ es top ctx τ Γ' D'} :
      JudgeSeq D Γ es top ctx τ Γ' D' →
      Judge D Γ (.seq es) top ctx τ Γ' D'

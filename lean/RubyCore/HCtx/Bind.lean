/-
  ...
  **The question.** `ratchet/Denote/Sem/Decompose.lean`'s `run_split` and
  `ratchet/Denote/Rules/VasgnStuck.lean`'s `stuckFreeRun_pushK_le` are two
  ...
  *context-carrying* continuation predicate (which also repairs the false
  `JumpStuckFree`, see `ratchet/Denote/Rules/WhileStuck2.lean`), or budget for a
